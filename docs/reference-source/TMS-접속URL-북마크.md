...
> **일지 팀 공유 SoT (2026-09-23):** Production도 Supabase 자동 미러(`MANUAL_MIRROR`)로 전환. A/B/C 모두 본인 URL에서 편집하면 ~8초 후 자동으로 클라우드에 저장됩니다 — 「팀 공유 저장」 버튼은 즉시 반영하고 싶을 때 쓰는 수동 보조 수단으로 남습니다. journal Blob POST는 자동 demote(GET·JSON 백업은 유지).  
...
**자동 저장 (2026-09-23~):** 일지 작성 후 입력을 멈추면 ~8초 뒤 자동으로 클라우드(Supabase)에 저장됩니다. 화면에 뜨는 「Supabase 자동 미러 대기/저장 중/완료」 문구가 그 상태 표시입니다. 즉시 반영하고 싶을 때는 「팀 공유 저장」을 수동으로 눌러도 됩니다.
...
**자동 저장 (2026-09-23~):** 본인(B) 탭에서 일지 작성 후 입력을 멈추면 ~8초 뒤 자동으로 클라우드(Supabase)에 저장됩니다 — 매번 버튼을 누를 필요는 없습니다.

**팀 공유:** 즉시 반영하고 싶을 때는 **「팀 공유 저장」**을 수동으로 눌러도 됩니다. 타인(A/C) 일지는 **A · C 탭**에서 **조회** — 비어 있으면 **「팀 공유본 가져오기」**(본인 일지는 유지, 타인만 갱신). JSON 백업·**「조회용 JSON 가져오기」**는 예비 수단입니다.
...
**일지:** B와 동일 — **A/B/C 탭 조회**, **C 탭만** 편집. **자동 저장(2026-09-23~)**: 입력 멈춘 후 ~8초 뒤 자동 클라우드 저장. 즉시 반영은 **「팀 공유 저장」**, 타인 조회는 **「팀 공유본 가져오기」**.  
...
구성원 URL(`/yhkim` 등) 툴바 우측 **「관리자」** 바로가기로 `/admin`에 이동할 수 있습니다. 진입 시 **비밀번호 게이트**가 필요합니다.  
관리자 URL(`/admin`) 툴바 우측 **「구성원 A」** 바로가기로 `/yhkim`(팀장 본인 일지·업무)에 이동할 수 있습니다.
...
- 일지·KPI는 **브라우저 localStorage**(즉시 편집면) + (운영) **클라우드 팀 공유**. 기기·브라우저마다 localStorage는 다를 수 있습니다.
- **일지 자동 저장 (2026-09-23~):** A/B/C 모두 본인 탭 편집 시 ~8초 후 자동으로 Supabase에 저장됩니다. 팀 공유의 기준(SoT)은 Supabase이고, Blob은 수동 백업·재해복구용으로만 남습니다.
- **구성원 일지 조회:** B/C도 A/B/C 탭 전환 가능. 타인 내용은 **「팀 공유본 가져오기」**로 갱신(본인 슬라이스 유지). 급하게 바로 반영하고 싶을 때는 **「팀 공유 저장」**을 수동으로 눌러도 됩니다.
