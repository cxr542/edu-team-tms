...
**일지:** 화면 상단 **A · B · C** 탭으로 팀원 일지를 **조회**할 수 있습니다. **본인(B) 탭만** 편집·승인 요청 가능하고, 다른 탭에는 **「조회」** 배지가 붙습니다. 팀원 PC에는 보통 **본인 일지만** 저장되므로, 다른 탭이 비어 있으면 팀장이 병합한 데이터가 이 브라우저에 없는 것입니다(팀장 PC에서 B·C 백업 병합 후 확인).
...
- **구성원 일지 조회:** B/C도 A/B/C 탭 전환은 가능하지만, **이 브라우저에 저장된 일지**만 보입니다. 팀원 PC에는 대개 본인 데이터만 있고, 팀장이 `백업 가져오기`로 모은 **전체 팀 일지**는 팀장 브라우저에서 확인합니다.
