...
이번 SQL 초안에서는 RLS를 활성화하고, 현재 1단계 병행 저장을 위한 draft select/insert/update policy를 기본으로 둔다. 단, 공지사항은 공개 공지만 anon 조회하고 초안 조회·등록·수정은 authenticated admin으로 제한한다. 실제 production write policy는 후속 PR에서 인증 전략을 확정한 뒤 더 좁게 설계한다.
