...
### 5단계. 실시간 구독 → J7e (알림·감사)

브라우저 Realtime on `journal_snapshots`는 admin-session 구조상 **보류** (anon SELECT 없음, 매직링크 필수화 안 함).

채택 경로 ([`j7-journal-realtime-blob-plan.md`](./j7-journal-realtime-blob-plan.md)):

- **J7a (완료):** `GET /api/journal-snapshots` ~30s·focus 폴링으로 「원격이 더 최신」만 갱신
- **J7e:** journal upsert 성공 시 `sync_events`에 감사 이벤트(`source=journal`, `member_code`, `updated_at`) 기록. UI는 「원격 갱신됨」 배지 → **수동 pull(J5)**. 자동 병합 없음
- 진짜 websocket(Auth 구독 또는 admin-session SSE)은 J7e 이후 별 PR

### 6단계. Blob 역할 축소 → J7b–J7d

단일 Blob `journal/live-latest.json` RMW는 교차 구성원 lost-update 위험이 있다 ([`journal-live-latest-concurrency-analysis.md`](./journal-live-latest-concurrency-analysis.md)). J7 본선은 per-member Supabase로 팀 공유 SoT를 옮긴다.

| 단계 | 내용 |
|------|------|
| **J7b** | Member referer 게이트로 `/api/journal-snapshots` 쓰기 허용 + Blob/Supabase **dual-write** (Preview) |
| **J7c** | 「팀 공유본 가져오기」 **Supabase-first**, Blob fallback |
| **J7d** | Journal Blob **POST demote** (GET은 재해 복구용 유지) |
