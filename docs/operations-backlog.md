...
| **J7b** | Member-scoped Supabase API + dual-write | Preview · 다음 |
| **J7c** | Pull SoT flip (Supabase-first) | Preview |
| **J7d** | Journal Blob POST demote | Preview 파일럿 후 |
| **J7e** | `sync_events` 감사 + 알림(폴링 연동) | 얇게 · 자동 merge 없음 |
...
**J7-0 완료:** Realtime·Blob 축소 로드맵 문서화 — [`j7-journal-realtime-blob-plan.md`](./j7-journal-realtime-blob-plan.md). 본선은 Blob→Supabase per-member 팀 공유 이전(J7b–J7d). Realtime은 알림·`sync_events` 감사(J7e). 다음은 **J7b**.
...
| **J7b** | Member-scoped API + Blob/Supabase dual-write | 개발 | Preview B/C 저장 시 양쪽 갱신 |
| **J7c** | Pull SoT flip (Supabase-first) | 개발 | Blob 없이도 peer pull |
| **J7d** | Journal Blob POST demote | 개발 | flag로 POST off, GET 유지 |
