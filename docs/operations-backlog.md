...
- **운영 DB:** `phase0-apply.sql` 적용 완료 (2026-07-06) — `announcements` 테이블 정상
...
| 단계 | 작업 | 목표 |
|------|------|------|
| **J1** | PR: `SupabaseAuthControls` + phase0 문서·SQL | 운영 `/admin` 매직링크 UI |
| **J2** | Auth Redirect + 팀장 매직링크 + `tms_profiles` admin | RLS 통과 가능 |
| **J3** | Preview `MANUAL_MIRROR=true` | A/B/C 일지 「Supabase 백업」 성공 |
| **J4** | 신선도 비교 UI (읽기 전용) | 원격 vs 로컬 `updated_at` 표시 |
| **J5** | 수동 가져오기 + 충돌 UX | 팀장이 Supabase → local 복구 |
| **J6** | 자동 미러 (저장 시 debounce) | 수동 버튼 없이 백업 |
| **J7** | Realtime·Blob 축소 | 장기 |
...
상세: §9 · [`journal-supabase-sync-plan.md`](./journal-supabase-sync-plan.md)
...
| 3 | Auth Redirect URL (`/**`) | ☐ | 매직링크 복귀용 — **다음 수동 작업** |
| 4 | 팀장 매직링크 가입 | ☐ | 구성원 전원 불필요 (팀장만으로 일지 미러 가능) |
| 5 | `tms_profiles` admin 1건 | ☐ | #4 이후 SQL |
| 6 | `npm run verify:supabase` | ☐ | 로컬 `.env.local`로 선택 검증 |
| 7 | 운영 QA (공지·헬스) | ✅ | 점검 정상, 공지 오류 없음 |

**저장소 (로컬, main 미반영):** `phase0-apply.sql`, `supabase-phase0-runbook.md`, `provision-tms-profiles.sql`, `verify:supabase` — **PR 필요**

**운영 앱(main) 갭:** `SupabaseAuthControls`(매직링크 UI) **미배포** → `/admin`에 이메일 로그인 버튼 없음 (의도된 상태)
...
| 공지·CSR | Supabase (anon RLS) | **운영 중** |
| 일지 Supabase 백업 | `journal_snapshots` | 코드 있음, **미러 플래그 off** |
...
J1 Auth UI·문서 PR   ░░░░░░░░░░   0%  ← 지금
J2 팀장 Auth·프로필  ░░░░░░░░░░   0%
J3 Preview 미러 파일럿 ░░░░░░░░░░   0%
J4 신선도 UI         ░░░░░░░░░░   0%
J5~J7 가져오기·자동화 ░░░░░░░░░░   0%

(인프라: journal_snapshots DDL ✅ · env ✅ · 코드 스켈레톤 ✅)
```
...
| 단계 | 작업 | 담당 | 완료 기준 |
|------|------|------|-----------|
| **J1** | PR: `SupabaseAuthControls` + `phase0-apply.sql`·런북·`.env.example` | 개발 | main 머지·배포 후 `/admin` 로그인 UI |
| **J2** | Redirect `/**` + 팀장 매직링크 + `tms_profiles` admin SQL | 운영 | 팀장 세션 + admin 프로필 |
...
| **J6** | local 저장 시 자동 미러 (debounce) | 개발 | 수동 버튼 없이 upsert |
| **J7** | Realtime 알림, Blob 역할 축소 | 개발 | 장기 |

**지금 막힌 이유:** J1 미배포 → `/admin`에 Supabase 로그인 없음 → J2·J3 불가.
...
- `src/utils/supabaseJournalSnapshot.js`, `SupabaseAuthControls.jsx` (로컬 브랜치)
