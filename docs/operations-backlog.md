...
공지사항 UI는 릴리즈노트형 타임라인 피드로 운영한다. 사용자 대면 변경 안내는 **공지(`release` 기본)** 에 등록하고, `docs/reference-source/TMS-릴리즈노트.md` 누적 이력은 선택적으로 보강한다.
...
**J3 완료 (2026-07-09):** Preview `MANUAL_MIRROR=true` + [`j3-grant-service-role-journal.sql`](../supabase/j3-grant-service-role-journal.sql) GRANT + A/B/C 「Supabase 저장 완료」 확인. 다음은 **J4** 신선도 UI.
