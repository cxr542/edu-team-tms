...
공지사항 UI는 릴리즈노트형 타임라인 피드로 운영한다. 사용자 대면 변경 안내는 **공지(`release` 기본)** 에 등록하고, `docs/reference-source/TMS-릴리즈노트.md` 누적 이력은 **main 머지 시 날짜별 merge**(`release-notes-on-merge.yml` → `## YYYY-MM-DD` + `###` PR 제목)하며 수동 보강도 가능하다. Deploy 재트리거를 위해 repo secret `RELEASE_NOTES_PUSH_TOKEN`(PAT)을 권장한다(`GITHUB_TOKEN`만 쓰면 문서 커밋은 되지만 Deploy가 자동 시작되지 않음).
