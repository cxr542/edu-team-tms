...
| **J7c** | Pull SoT flip (Supabase-first) | Preview · 다음 |
...
**J7b 완료:** member referer로 `/api/journal-snapshots` GET/POST 허용. B/C 「팀 공유 저장」이 Preview `MANUAL_MIRROR`일 때 Blob 성공 후 Supabase dual-write(best-effort). 빈 일지 덮어쓰기 가드. 다음은 **J7c**.
