...
| 데이터 | 저장소 | 운영 상태 |
|--------|--------|-----------|
| 업무일지 | localStorage + Blob 수동 공유 | **주 저장소** (2026-09-23 롤백 후 원복) |
...
| **J8b** | Production `MANUAL_MIRROR` + 자동 | 롤백 (2026-09-23) | Vercel Production env 삭제 + 재배포로 원복. 사유: `journal_snapshots`가 Blob 대비 얕아 「가져오기」가 얕은 데이터로 로컬을 덮어쓸 위험 발견 |
