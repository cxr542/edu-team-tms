...
describe('kpiOperationalSupabase', () => {
  ...
  it('loads a KPI1 monthly approval row from Supabase', async () => {
    ...
    expect(from).toHaveBeenCalledWith('kpi_monthly_approvals');
    expect(select).toHaveBeenCalledWith('member_code, year_month, monthly01, payload_version, updated_at');
  ...
  it('saves and loads a KPI2 approval row from Supabase', async () => {
    ...
    expect(loadSelect).toHaveBeenCalledWith('member_code, day_key, task_id, kpi2_row_status, payload_version, updated_at');
